...
RegisterNetEvent("edz-blackout:baslat")
AddEventHandler("edz-blackout:baslat", function()
...
RegisterNUICallback('close', function(data, cb)
  ...
  exports['mythic_notify']:SendAlert('error', 'Sisteme giremedin!')
  cb('ok')
end)
...
RegisterNUICallback('complete', function(data, cb)
  ...
  ClearPedTasks(PlayerPedId())
  Citizen.Wait(2500)
  exports['mythic_notify']:SendAlert('inform', 'Sistem devredışı!')
  Elektrigikes()
  cb('ok')
end)
...
function Elektrigikes()
  ...
  local giveAnim = "anim@heists@ornate_bank@hostages@hit" --> Here is your animLib that u want use.
	...
	TriggerServerEvent('vSync:elektrik')
	Citizen.Wait(900000)
	TriggerServerEvent('vSync:elektrik')
end

RegisterNetEvent('edz-blackout:status')
AddEventHandler('edz-blackout:status', function(status)
